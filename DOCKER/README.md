...
# What is docker
1. 읽기 전 참고 용어
    0. Hypervisor : 가상화 기술에서 사용되는 기술. 가상 머신을 생성,관리 하기 위해 HOST OS에 설치 됨.
    1. 가상 머신 : Hypervisor로부터 생성 된 OS(운영체제). Ubuntu,CentOS,Windows 등이 해당됨
    2. HOST OS : 서버를 부팅할 때 실행되는 운영체제를 의미. 쉽게 말해 컴퓨터 자체를 킬 때 최초로 실행되는 OS
    3. GUEST OS : HYPERVISOR에 의해 운영체제가 생성(설치)되고 관리되는 운영체제를 의미
        - 각 GUEST OS 끼리는 완전히 독립된 공간과 시스템 자원을 할당 받음.
        - VMWARE , VirtualBox 프로그램이 대표적 GUEST OS 방식으로 OS를 생성

2. Hypervisor가 관리하는 시스템의 장단점
    - 장점
        1. 가상 머신은 완벽한 운영체제 생성이 가능.
    - 단점 
        1. 가상 머신은 Geust OS를 사용하기 위한 라이브러리, 커널 등을 전부 포함하기 때문에 용량이 큼
        2. Host OS에 비해 성능 손실이 존재한다.
3. Docker가 관리하는 시스템의 장단점
    - 장점
        1. 리눅스 자체 기능인 chroot,namespace,cgroup을 사용하기에 process 단위의 격리 환경을 만들어 성능 손실이 거의 없음
        2. docker container 는 HOST OS 를 통해 실행되지만 "확실하게 HOST OS와 분리된 공간이다". 즉 컨테이너 자체에 특별 권한이 주어지지 않는 한 컨테이너에 있는 수 많은 SOFTWARE가 HOST OS에 영햐을 끼치지 않는다.
    - 단점
...
- 사용법
    ...
    ### 이미지-추출/로드/배포
        - 이미지 추출 : docker save _이미지이름_ . 컨테이너의 cmd , 이미지 이름 , 태그 등의 모든 메타데이터를 포함하여 하나의 파일로 추출가능.
            - option
                1. docker save -o savename.tar myserverimage:first , myserverimage:first 이미지를 savename.tar로 추출한다
        - 이미지 로드 : docker load -i savename.tar , savename.tar 이미지를 로드 추출한 이미지와 완전히 동일한 이미지가 docker engine에 생성

        - 이미지 배포 : 

        - 이미지 이름 변경 및 이미지를 가르키는 이름추가
            1. docker tag _기존이미지이름_:태그 _새롭게할당할이름_:태그 , 이미지의 이름을 변경했다고 해서 기존 이름이 사라지지 않음. 같은 이미지를 가르키는 새로운 이름을 추가할 뿐
    ### 컨테이너 생성
